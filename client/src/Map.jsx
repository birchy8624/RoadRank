  ...
  useEffect(() => {
    async function fetchRoads() {
      try {
        ...
        const roadsWithParsedPaths = response.data.map((road) => ({
          ...road,
          path: JSON.parse(road.path),
        }));
        setRoads(roadsWithParsedPaths);
      } catch (error) {
  ...
  return (
    <>
      ...
      <GoogleMap
        ...
        onMouseUp={async () => {
          if (drawing && drawnPath.length > 1) {
            const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/snap-to-road`, { path: drawnPath });
      ...
      >
        ...
        {selectedRoad && (() => {
          const middleIndex = Math.floor(selectedRoad.path.length / 2);
          return (
            <InfoWindow
              position={selectedRoad.path[middleIndex]}
